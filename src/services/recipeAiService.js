...
const runRebuttalAndRevise = async ({ provider, draftParsed, contextBlock, memoryContext, directionContext, rebuttalIndex, onProgress }) => {
    ...
    // 反証エージェントが弱点を指摘したか＝実際に「差し戻し（否決）」が発生したかを判定する。
    // 指摘なし＝そのまま採用（可決）、指摘あり＝統括シェフへ差し戻して再修正。
    const reworked = hasRebuttalCritique(rebuttalOutput);

    if (!reworked) {
        onProgress?.({ type: 'audit-verdict', verdict: 'approve' });
        return { finalParsed: draftParsed, rebuttalOutput, reworked: false };
    }

    // 差し戻しの演出：まず「否決」を見せる → 少し待って審議画面（帯上下）へ戻す →
    // 再修正が終わったら次工程へ進むタイミングで「可決」にする。
    onProgress?.({ type: 'audit-verdict', verdict: 'reject' });
    await new Promise((resolve) => setTimeout(resolve, 2400));
    onProgress?.({ type: 'audit-verdict', verdict: 'rework' });

    let finalParsed = draftParsed;
    try {
    ...
    // 反証・再修正が完了＝次工程へ。可決にする。
    onProgress?.({ type: 'audit-verdict', verdict: 'approve' });
    return { finalParsed, rebuttalOutput, reworked: true };
};
...
export const generateProductRecipeDraft = async ({ brief, provider, directionContext = '', onProgress }) => {
...
export const generateRecipeImprovement = async ({ recipe, notes, provider, directionContext = '', onProgress }) => {
...
- Return strict JSON only. 以下の構造の json 形式でのみ出力してください。
{
...
export const continueRecipeAiConversation = async ({
    ...
    // クロスチェック（検証AI）が指摘を出したか＝差し戻し（否決）が発生したかを判定する。
    // 指摘あり＝否決を見せて審議画面に戻し、最終統合（再修正）後に可決へ。
    if (hasRebuttalCritique(auditOutput)) {
        onProgress?.({ type: 'audit-verdict', verdict: 'reject' });
        await new Promise((resolve) => setTimeout(resolve, 2400));
        onProgress?.({ type: 'audit-verdict', verdict: 'rework' });
    }
