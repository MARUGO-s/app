...
export const RecipeForm = ({ onSave, onCancel, initialData }) => {
    ...
    // 検証AI（反証）の実際の合否。'approve'（可決）/ 'reject'（否決＝差し戻し）/ null（未判定）
    ...
    // 通信完了後、進捗を最終工程まで進めて可決表示を一定時間見せてから閉じる。
    // これをせずに即クローズすると、分析完了と同時にポップアップが急に落ちて消えてしまう。
    const finishAiProgress = async (mode) => {
        const stepCount = getRecipeAiProgressConfig(mode)?.steps?.length || 1;
        setAiProgressStepIndex(Math.max(stepCount - 1, 0));
    ...
    const handleGenerateAiDraft = async () => {
        ...
        try {
            const draft = await generateProductRecipeDraft({
                ...
                onProgress: (event) => {
                    if (event?.type === 'audit-verdict') setAiAuditVerdict(event.verdict);
                },
            });
            // 完了演出：最終工程まで進めて可決表示を見せてから閉じる（分析完了時に急に落ちないように）
            await finishAiProgress('product-generate');
            setAiDraft(draft);
    ...
    const handleApplyConversationToDraft = async () => {
        ...
        try {
            ...
            await finishAiProgress('product-conversation');
