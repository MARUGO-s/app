...
const SortableStepItem = ({
    ...
    return (
        <div ref={setNodeRef} style={style} className="step-row" style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem', alignItems: 'flex-start' }}>
            <div
                {...attributes}
                {...listeners}
...
const SortableSection = ({ section, sections, onSectionChange, onRemoveSection, children }) => {
    return (
        <Card className="step-section mb-md" style={{ border: '1px solid #e0e0e0', boxShadow: 'none' }}>
            ...
            <div className="section-steps-list">
                {children}
            </div>
        </Card>
    );
