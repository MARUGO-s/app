...
### レシピ一覧の表示改善
- **カード / リスト切り替え**: フィルター横のボタンで、サムネイル付きカード表示とテーブル形式（画像なし・軽量）を切り替え可能。選択はブラウザに保存されます。
- **全表示の挙動修正**: 「全表示」は表示件数の展開のみ（カード表示を維持）。リスト表示とも併用できます。
- **一般ユーザー向け一覧修正**: マスターレシピ共有ON時、共有レシピもメインのコース別一覧に表示。件数と表示のずれを解消。
- 操作質問・`public/recipe.html`・`docs/` のガイドを同期更新。
...
### 資料箱（ファイル保管）機能
- **ファイル資料の保管**: ユーザーごとにファイルを保存できる資料箱を追加
- **ドラッグ&ドロップ対応**: ファイルをドロップして簡単に登録可能
- **圧縮保存**: 添付ファイルは圧縮して保存し、必要時に解凍ダウンロード可能
- **プレビュー対応**: 画像・PDF・テキスト・音声・動画のプレビュー表示に対応
- **容量可視化**: ユーザーごとに 10MB 上限を適用し、使用量をバー表示
- **ファイル検索強化**: ファイル名・カテゴリーで検索/絞り込み可能
- **カテゴリー管理**: アップロード時のカテゴリー設定と、既存ファイルの後編集に対応
- **管理者共有機能**: 個別共有・一覧一括共有・共有先選択UIを追加（ポップアップ表示）
- **共有状態表示**: 共有済み人数・最終共有時刻を表示し、管理状態を確認可能

### 合成原価の分析・逆算・変動確認
- **原価インパクト分析**: 合成原価に占める割合を行ごとに表示し、どのレシピ・材料が原価を押し上げているかを高い順に確認可能
- **目標原価率から使用量を逆算**: 販売価格と目標原価率を入力し、対象行以外の原価を固定したうえで、選択したレシピ/材料の上限使用量を逆算可能
- **逆算値の反映**: 算出した上限使用量をボタンでそのまま使用量欄へ反映可能
- **保存済み一覧の原価変動バッジ**: 保存時の合成原価と現在の材料単価・カテゴリ原価で再計算した原価との差分を一覧で確認可能
- **変動ステータス表示**: 差分なし、上昇、低下、取得不可、一部取得不可をカード上で判別可能

### 合成レシピのバージョン確認強化
- **復元前プレビュー**: 保存済み合成レシピの各バージョンを、復元する前に内容確認可能
- **現在編集中との比較表示**: 選択した過去バージョンと、現在編集中の内容差分を画面上で比較可能
- **読み取り専用表示**: 過去バージョンを復元せず、そのまま読み取り専用で開いて確認可能
- **復元判断がしやすいUI**: 料理名、ベースレシピ、合成原価、共有状態、組み合わせ内容を事前に確認できるため、誤って戻すリスクを減らせる

## 🆕 新機能追加（2026-05-01）

### 合成レシピ保存・編集機能
- **合成原価ページから保存**: 合成した組み合わせに料理名を付けて保存可能
- **保存済み合成レシピ一覧ページ**: 保存した構成を一覧表示し、いつでも再利用可能
- **削除機能**: 保存済み合成レシピを一覧から削除可能
- **編集ページ**: 保存済み構成を開いて、各レシピの使用量（グラム等）を再編集可能

### 合成原価表示の改善
- **編集ページでも原価・総出来上がり量を正しく表示**: 保存済みデータ読込時に必要なレシピ詳細を自動取得し、`—` 表示を解消
- **原価率計算の修正**: 販売数を変えても原価率が不自然に変動しないよう計算式を修正
  - 原価率: `1個あたり原価 ÷ 販売価格`
  - 粗利益: `(販売価格 × 販売数) - (1個あたり原価 × 販売数)`
- **カテゴリ別原価の単体試算に対応**: 1つのレシピ内でカテゴリ（グループ）ごとに原価を算出・再設定できる機能を整備
- **カテゴリ別原価と合成原価の連携**: 単体レシピ側で設定したカテゴリ原価（再設定値）を、合成レシピの原価計算にも反映可能
  - 未設定カテゴリは元データ原価を自動利用
  - 設定済みカテゴリは再設定原価を優先して合成計算に反映

### 合成レシピ作成フローの改善
- **固定ベース前提を廃止**: 検索結果から連続して組み合わせ追加可能
- **先頭レシピも入れ替え・削除可能**: 最初に選んだレシピも柔軟に変更できるUIへ
- **不要UIを整理**: 「ベースレシピ」選択欄と「+ 組み合わせを追加」ボタンを廃止し、検索カード中心の操作に統一

### UI改善（合成原価関連ページ）
- 保存済み合成レシピ一覧のカードデザインを改善（視認性・情報整理・操作性向上）
- 合成原価シミュレーターの入力/結果エリアの視認性を改善（フォーカス、カード構造、情報強調）
- 保存済み合成レシピページに**トップページへ戻る導線**を追加
...
- **カテゴリ別原価の見える化**: レシピ詳細でカテゴリ見出しを押すと、カテゴリ単位で原価を確認可能
  - 材料数、単位別分量、総重量、総液量をまとめて表示
  - 原価小計、税込原価、カテゴリ原価率をポップアップで即確認
  - 「どの仕込み・どのパーツに原価がかかっているか」が把握しやすくなり、原価の見通しが大きく改善
- **カテゴリ使用量シミュレーション**: 仕込み全体から一部だけ使う場合の原価を概算
  - 例: ムースを 20g 使った時の原価（税込）をその場で計算
  - 総出来上がり量には、重量(g)と液量(ml)が混在する場合でも `1ml = 1g` の概算で自動入力
  - `個` や `本` などは自動換算に含めず、必要に応じて実際の仕上がり量へ手動調整
...
- **バックアップ管理（管理者）**: 全ユーザーのレシピをサーバーに最大3世代保存。**毎週1回**自動実行（月曜 0:00 JST）+ 手動実行・復元
...
#### AI API保護機能 (Google Cloud API)
- **Edge FunctionsでのJWT検証**:
  Gemini等のAPIを呼び出すSupabase関数（`call-gemini-api`, `analyze-image`等）は、アプリ上で認証済み（ログイン済み）のユーザーから送信された有効なJWT（アクセストークン）がない限り実行されません（401エラー）。これにより、外部ボット等によるAPIの不正利用（タダ乗り）を完全にブロックしています。
- **APIキーの用途制限**:
  Google Cloudコンソールにて、APIキーに「Generative Language API」のみの制限を付与しています。
  *※Geminiのマルチモーダル機能（画像とテキストの両方の処理）はすべてこの1つのAPI枠組みに含まれるため、画像解析とテキスト解析の両方がこの設定で安全に運用されます。万が一別の機能に転用しようとしても、Google側で強制的にブロック（403エラー）されます。*
...
1. メニューから **「🥪 合成原価」** を開く
2. レシピ検索・材料検索で組み合わせる項目を追加
3. 使用量を入力すると、合成原価合計と行別の原価インパクトが表示される
4. 販売価格と目標原価率を入力すると、必要売価や対象行の上限使用量を逆算可能
5. 保存済み合成レシピ一覧では、保存時原価と現在再計算原価の差分を `原価変動` バッジで確認可能
...
1. **データ管理** → **バックアップ管理** タブ（管理者のみ）
2. ユーザーごとに第1〜第3世代のバックアップを確認
3. **今すぐバックアップ** で手動実行、**復元** / **DL** で世代単位の操作

自動バックアップは **毎週1回**（**月曜 0:00 JST**）です。詳細は [docs/account-backup.md](./docs/account-backup.md) を参照してください。
...
**Q: データベースのバックアップ戦略は？**
A: 次の2層があります。(1) **Supabase** のプロジェクト全体スナップショット（約24時間ごと）。(2) アプリの **バックアップ管理** で、ユーザーごとのレシピを最大3世代保持（**毎週1回**・月曜 0:00 JST に自動実行）。詳細は [docs/account-backup.md](./docs/account-backup.md)。
...
- [アカウント別バックアップ](./docs/account-backup.md) - 管理者向け世代バックアップ（週次自動・手動・復元）
