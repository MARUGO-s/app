...
const formatGeminiError = (status: number, errText: string) => {
  ...
  try {
    ...
    return `Gemini API Error: ${status} ${
      retryLine && retryLine !== first
        ? `${first} (${retryLine.replace(/\.$/, "")})`
        : first
    }`;
  } catch {
    // ignore
  ...
  const snippet = trimmed.length > 600
    ? `${trimmed.slice(0, 600)}...`
    : trimmed;
  if (status === 429) {
...
async function analyzeImageWithGemini(
  ...
  const apiKey = Deno.env.get("GEMINI_API_KEY") ||
    Deno.env.get("GOOGLE_API_KEY") || Deno.env.get("VISION_API_KEY");
  if (!apiKey) {
    console.warn("Skipping Gemini: No API Key found");
    return null;
  }

  const modelCandidates = resolveGeminiModelCandidates().filter((m) =>
    !isBlockedGeminiModel(m)
  );
  ...
  try {
    ...
    if (file.size > MAX_GEMINI_IMAGE_BYTES) {
      return {
        error: `画像サイズが大きすぎるためGeminiをスキップします (${
          Math.round(file.size / 1_000_000)
        }MB)`,
      };
    }
    ...
    try {
      for (const modelId of modelCandidates) {
        for (const apiVersion of apiVersions) {
          const endpoint =
            `https://generativelanguage.googleapis.com/${apiVersion}/models/${modelId}:generateContent`;
...
async function analyzeImageWithAzure(file: File) {
  ...
  if (!AZURE_DI_KEY || !AZURE_DI_ENDPOINT) {
    return {
      error:
        "Azureの認証情報が設定されていません（AZURE_DI_ENDPOINT / AZURE_DI_KEY もしくは AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT / AZURE_DOCUMENT_INTELLIGENCE_KEY）",
    };
  }

  try {
    const apiUrl =
      `${AZURE_DI_ENDPOINT}/formrecognizer/documentModels/prebuilt-layout:analyze?api-version=2023-07-31`;
    ...
    if (!operationLocation) {
      return {
        error: "Azureからの応答に Operation-Location が含まれていません。",
      };
    }
...
async function analyzeImageWithOpenAiVision(
  ...
  const apiKey = Deno.env.get("OPENAI_API_KEY") || Deno.env.get("chatgpt") ||
  ...
  const overrideModel = String(Deno.env.get("OPENAI_VISION_MODEL") || "")
    .trim();
  ...
  try {
    ...
    if (file.size > MAX_VISION_IMAGE_BYTES) {
      return {
        error: `画像サイズが大きすぎるためOpenAI(Vision)をスキップします (${
          Math.round(file.size / 1_000_000)
        }MB)`,
      };
    }
    ...
    if (base64Image.length > MAX_VISION_BASE64_CHARS) {
      return {
        error: `画像が大きすぎるためOpenAI(Vision)をスキップします (base64=${
          Math.round(base64Image.length / 1_000_000)
        }MB)`,
      };
    }
    const mimeType = normalizeImageMimeType(file);
...
`;
    ...
    if (!response.ok) {
      ...
      return {
        error: `OpenAI(Vision) API Error: ${response.status} ${errText}`,
      };
    }
    ...
    try {
      ...
      if (!hasContent) {
        return {
          error:
            "OpenAI(Vision)のJSONにレシピ内容（材料・手順）がありませんでした",
      ...
      return {
        error: `OpenAI(Vision)のJSON解析に失敗しました: ${getErrorMessage(e)}`,
        rawText: String(content).slice(0, 20_000),
      };
    }
  } catch (e) {
...
function normalizeRecipeFromLLM(recipe: any) {
  ...
  normalized.ingredients = rawIngredients.map((ing: any) => {
    ...
    const name = String(
      ing.name ?? ing.item ?? ing.material ?? ing.ingredient ?? "",
    ).trim();
    const quantity = String(ing.quantity ?? ing.qty ?? "").trim();
    const unit = String(ing.unit ?? "").trim();
    const group = ing.group == null || ing.group === ""
      ? null
      : String(ing.group).trim();
...
function ensureRecipeTitle(recipe: any): { recipe: any; hasContent: boolean } {
  ...
  if (!title && hasContent) {
    ...
    title = (firstGroup && String(firstGroup).trim()) ||
      (firstIngName && String(firstIngName).trim()) ||
      "画像から取り込んだレシピ";
  }
...
async function analyzeRecipeTextWithGroq(
  ocrText: string,
  supabaseClient: any,
  ctx: RequestLogContext,
) {
  ...
  const modelId = String(Deno.env.get("GROQ_RECIPE_MODEL") || "").trim() ||
    "openai/gpt-oss-120b";
  ...
  const shortText = trimmedText.length > MAX_OCR_CHARS
    ? `${trimmedText.slice(0, MAX_OCR_CHARS)}\n...`
    : trimmedText;
  if (!shortText) {
    return { error: "OCRテキストが空のためGroq解析をスキップします" };
  }
...
`;
  ...
  try {
    ...
    if (!hasContent) {
      return {
        error: "GroqのJSONにレシピ内容（材料・手順）がありませんでした",
        rawText: String(content).slice(0, 20_000),
      };
    }
    ...
    return {
      error: `GroqのJSON解析に失敗しました: ${getErrorMessage(e)}`,
      rawText: String(content).slice(0, 20_000),
    };
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      userEmail = typeof (payload as any).email === "string"
    ...
    try {
      await enforceApiRateLimit(userId, "analyze-image", {
        maxRequests: 60,
        windowMinutes: 60,
      });
      multipartBytes = await readRequestBytesLimited(req, MAX_MULTIPART_BYTES);
    } catch (error) {
      if (
        error instanceof ApiRateLimitError || error instanceof HttpRequestError
      ) {
    ...
    const clientIp = clientIpHeader
      ? String(clientIpHeader).split(",")[0].trim()
      : null;
    ...
    try {
      ...
      console.warn(
        "Failed to init Supabase client (logging will be disabled):",
        getErrorMessage(e),
      );
      supabaseClient = null;
    }
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ error: "multipart形式が不正です。" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const declaredImageType = String(imageFile.type || "").toLowerCase();
    const hasKnownImageExtension = /\.(?:jpe?g|png|webp|gif|heic|heif)$/i.test(
      imageFile.name || "",
    );
    if (
      imageFile.size <= 0 ||
      ...
      (declaredImageType
        ? !ALLOWED_IMAGE_TYPES.has(declaredImageType)
        : !hasKnownImageExtension)
    ) {
      return new Response(
        JSON.stringify({ error: "画像の形式またはサイズが不正です。" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const engineRaw = String(formData.get("engine") || "groq").trim()
      .toLowerCase();
    const engine = (engineRaw === "gemini" || engineRaw === "groq" ||
        engineRaw === "groq_vision" || engineRaw === "auto")
      ? engineRaw
      : "auto";
    ...
    if (engine === "gemini") {
      ...
      if (!geminiKey) {
        const msg =
          "Gemini API Keyが設定されていません（GEMINI_API_KEY/GOOGLE_API_KEY）";
        logs.push(`⚠️ ${msg}`);
      ...
      if (supabaseClient) {
        try {
          const maxReq = Number(
            Deno.env.get("GEMINI_IMAGE_RATE_LIMIT_MAX") || 30,
          );
          const windowMin = Number(
            Deno.env.get("GEMINI_IMAGE_RATE_LIMIT_WINDOW_MIN") || 1440,
          );
          const limiter = new RateLimiter(
            supabaseClient,
            userId,
            "analyze-image:gemini",
            { maxRequests: maxReq, windowMinutes: windowMin },
          );
          await limiter.check();
        } catch (e) {
          logApiUsage(supabaseClient, {
          ...
          return sendError(getErrorMessage(e));
        }
      }
    ...
    // --------------------------------------------------------------------------
    if (engine === "groq") {
      const visionKey = Deno.env.get("OPENAI_API_KEY") ||
        Deno.env.get("chatgpt") || Deno.env.get("CHATGPT_API_KEY");
      ...
      if (visionKey) {
        ...
        const visionResult = await analyzeImageWithOpenAiVision(
          imageFile,
          supabaseClient,
          ctx,
        );
      ...
      if (hasAzure && groqKey) {
        ...
        const ocrText = String(
          (azureResult as any)?.fullText || (azureResult as any)?.rawText || "",
        ).trim();

        if (ocrText) {
          logs.push("🤖 Groqでレシピを構造化中...");
          const groqResult = await analyzeRecipeTextWithGroq(
            ocrText,
            supabaseClient,
            ctx,
          );
          if (groqResult?.recipe?.title) {
            ...
            return sendResult(
              groqResult.recipe,
              String((azureResult as any)?.rawText || "").slice(0, 20_000),
              "groq",
            );
          }
          ...
          const reason = (azureResult as any)?.error ||
            "Azure OCRに失敗しました";
          azureReason = `Azure OCR: ${reason}`;
          logs.push(`⚠️ ${azureReason}`);
        }
      } else {
        azureReason = !hasAzure
          ? "Azure OCR未設定"
    ...
    if (hasAzure) {
      ...
      const hasUsefulAzureRecipe = !!(
        ...
        ((azureRecipe?.ingredients?.length ?? 0) > 0 ||
          (azureRecipe?.steps?.length ?? 0) > 0)
      );
      if (hasUsefulAzureRecipe) {
        ...
        return sendResult(
          azureRecipe,
          String((azureResult as any)?.rawText || "").slice(0, 20_000),
          String((azureResult as any)?.source || "azure"),
        );
      }

      azureFailureReason = (azureResult as any)?.error ||
        "Azure解析に失敗しました";
...
function parseAzureResult(lines: string[], fullText: string) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const isIngLine = ingredientPattern.test(line) && line.length < 50 &&
      !isSentence;

    if (isStepLine) {
      const startsWithUnit = /^\d+\s*(g|ml|kg|cc|tbsp|tsp|cup|個|本|枚|円|%)/i
        .test(line);
