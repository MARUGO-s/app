...
const MAX_SCREENSHOT_JSON_BYTES = 18 * 1024 * 1024;
...
serve(async (req) => {
  ...
  try {
    await enforceApiRateLimit(userId, "screenshot-recipe", {
      maxRequests: 30,
      windowMinutes: 60,
    });
  } catch (error) {
  ...
  try {
    const requestPayload = await readJsonBodyLimited<Record<string, unknown>>(
      req,
      MAX_REQUEST_BYTES,
    );
    const { url, fullPage = true, viewportWidth = 1280, viewportHeight = 720 } =
      requestPayload;

    if (!url) {
      throw new HttpRequestError("URL is required", 400);
    }
    ...
    const normalizedOptions = {
      ...
      viewportWidth: clampDimension(viewportWidth, 1280, 320, 1920),
      viewportHeight: clampDimension(viewportHeight, 720, 240, 2160),
    };

    console.log(
      "📸 Screenshot request for:",
      formatPublicUrlForLog(validatedUrl),
    );
    console.log("🖥️ Viewport settings:", normalizedOptions);

    // 実際のWebサイトをJPEG形式でキャプチャ
    const screenshot = await captureWebsiteAsJPEG(safeUrl, normalizedOptions);
    ...
    return new Response(
      JSON.stringify({
        ...
        data: {
          title: "スクリーンショット",
          description: `${safeUrl}のスクリーンショット`,
          ingredients: [],
          steps: [],
        },
...
function xmlText(value: unknown, maxLength: number): string {
  return String(value ?? "")
    .replace(/[\u0000-\u001f\u007f]/g, " ")
    .slice(0, maxLength)
    .replace(/&/g, "&amp;")
...
async function captureWebsiteAsJPEG(
  url: string,
  options: ScreenshotOptions,
): Promise<string> {
  try {
    console.log(
      "🔍 Trying screenshot services for:",
      formatPublicUrlForLog(new URL(url)),
    );
    ...
    try {
      ...
      console.warn("⚠️ Screenshot.guru failed:", errorMessage(error));
    }
    ...
    try {
      ...
      console.warn("⚠️ ScreenshotAPI failed:", errorMessage(error));
    }
    ...
    try {
      ...
      console.warn("⚠️ Free screenshot service failed:", errorMessage(error));
    }
    ...
    throw new Error(`Screenshot capture failed: ${errorMessage(error)}`);
  }
}

// サービス1: screenshot.guru
async function tryScreenshotGuru(
  url: string,
  options: ScreenshotOptions,
): Promise<string> {
  ...
  if (
    ...
    data.image.length <= MAX_SCREENSHOT_JSON_BYTES
  ) {
...
// サービス2: screenshotapi.net
async function tryScreenshotAPI(
  url: string,
  options: ScreenshotOptions,
): Promise<string> {
  ...
  const { response } = await safeFetchPublicUrl(apiUrl, {
    timeoutMs: 20_000,
    headers: {
      "User-Agent":
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
...
// サービス3: 無料のスクリーンショットサービス
async function tryFreeScreenshot(
  url: string,
  options: ScreenshotOptions,
): Promise<string> {
  // thum.io (無料サービス)
  const apiUrl =
    `https://image.thum.io/get/width/${options.viewportWidth}/crop/${options.viewportWidth}/${options.viewportHeight}/${url}`;
...
async function generateFallbackJPEG(
  url: string,
  options: ScreenshotOptions,
): Promise<string> {
  try {
...
async function generateWebsiteJPEG(
  url: string,
  title: string,
  description: string,
  options: ScreenshotOptions,
  ...
  const hostname = xmlText(new URL(url).hostname, 253);
  const displayUrl = xmlText(
    url.length > 60 ? url.substring(0, 60) + "..." : url,
    64,
  );
  const displayTitle = xmlText(
    title.length > 50 ? title.substring(0, 50) + "..." : title,
    54,
  );
  const displayDescription = xmlText(
  ...
  const svg = `
    <svg width="${options.viewportWidth}" height="${options.viewportHeight}" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- URL bar -->
      <rect x="120" y="20" width="${
    options.viewportWidth - 140
  }" height="30" rx="15" fill="white" stroke="#dee2e6" stroke-width="1"/>
      <text x="135" y="40" class="url">${displayUrl}</text>
      
      <!-- Main content area -->
      <rect x="0" y="70" width="100%" height="${
    options.viewportHeight - 70
  }" fill="white"/>
      ...
      <text x="30" y="130" class="title">${displayTitle}</text>
      ...
      ${
    displayDescription
      ? `<text x="30" y="210" class="subtitle">${displayDescription}</text>`
      : ""
  }
      
      <!-- Content blocks -->
      <rect x="30" y="230" width="${
    options.viewportWidth - 60
  }" height="25" rx="4" fill="#e9ecef"/>
      <rect x="30" y="265" width="${
    (options.viewportWidth - 60) * 0.8
  }" height="25" rx="4" fill="#e9ecef"/>
      <rect x="30" y="300" width="${
    (options.viewportWidth - 60) * 0.95
  }" height="25" rx="4" fill="#e9ecef"/>
      
      <!-- Article content area -->
      <rect x="30" y="340" width="${
    (options.viewportWidth - 60) * 0.65
  }" height="120" rx="8" fill="#f8f9fa" stroke="#dee2e6"/>
      <rect x="${
    options.viewportWidth - 250
  }" y="340" width="220" height="120" rx="8" fill="#fff3cd" stroke="#ffeaa7"/>
      
      <!-- Text lines -->
      <rect x="45" y="360" width="${
    (options.viewportWidth - 60) * 0.55
  }" height="12" rx="2" fill="#dee2e6"/>
      <rect x="45" y="380" width="${
    (options.viewportWidth - 60) * 0.45
  }" height="12" rx="2" fill="#dee2e6"/>
      <rect x="45" y="400" width="${
    (options.viewportWidth - 60) * 0.6
  }" height="12" rx="2" fill="#dee2e6"/>
      <rect x="45" y="420" width="${
    (options.viewportWidth - 60) * 0.4
  }" height="12" rx="2" fill="#dee2e6"/>
      
      <!-- Footer -->
      <rect x="0" y="${
    options.viewportHeight - 50
  }" width="100%" height="50" fill="#343a40"/>
      <text x="${options.viewportWidth / 2}" y="${
    options.viewportHeight - 25
  }" text-anchor="middle" style="fill:white; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 12px;">
        © ${new Date().getFullYear()} ${hostname} - キャプチャ日時: ${
  ...
  const base64Svg = bytesToBase64(new TextEncoder().encode(svg));
...
async function generateSimpleJPEG(
  url: string,
  options: ScreenshotOptions,
): Promise<string> {
  ...
  const svg = `
    <svg width="${options.viewportWidth}" height="${options.viewportHeight}" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect x="50" y="50" width="${options.viewportWidth - 100}" height="${
    options.viewportHeight - 100
  }" fill="white" stroke="#dee2e6" stroke-width="2" rx="8"/>
      
      <text x="${
    options.viewportWidth / 2
  }" y="150" text-anchor="middle" style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 24px; font-weight: 600; fill: #495057;">
        スクリーンショット取得完了
      </text>
      
      <text x="${
    options.viewportWidth / 2
  }" y="200" text-anchor="middle" style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 16px; fill: #6c757d;">
        ${hostname}
      </text>
      
      <text x="${
    options.viewportWidth / 2
  }" y="250" text-anchor="middle" style="font-family: monospace; font-size: 12px; fill: #868e96;">
        ${displayUrl}
      </text>
      
      <text x="${options.viewportWidth / 2}" y="${
    options.viewportHeight - 50
  }" text-anchor="middle" style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 12px; fill: #adb5bd;">
        ${new Date().toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" })}
      </text>
    </svg>
  `;
