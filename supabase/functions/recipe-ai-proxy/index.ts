...
const ALLOWED_MODELS: Record<string, Set<string>> = {
  "sakana-subscription": new Set(["fugu"]),
  "sakana-payg": new Set(["fugu"]),
  sakana: new Set(["fugu"]),
  groq: new Set(["openai/gpt-oss-120b", "groq/compound"]),
  openai: new Set(["gpt-5.4-mini", "gpt-5.4-nano"]),
  perplexity: new Set(["sonar"]),
};

const DEFAULT_MODELS: Record<string, string> = {
  "sakana-subscription": "fugu",
  "sakana-payg": "fugu",
  sakana: "fugu",
...
const extractOpenAiWebSearchCallCount = (
...
const PROVIDER_ENDPOINTS: Record<
  string,
  Record<string, { url: string; keyEnvs: string[]; providerName: string }>
> = {
...
serve(async (req) => {
  ...
  const userEmail = typeof jwtPayload.email === "string"
    ? jwtPayload.email
    : null;
  if (!userId) {
  ...
  // 有料APIのためDB障害時も無制限にはせず、fail closed にする。
  try {
    ...
    const unavailable = String((rateLimitError as Error)?.message || "")
      .includes("一時的に利用できません");
    return jsonResponse({
      success: false,
      error: unavailable
        ? "利用制限の確認に失敗しました。しばらく待ってから再試行してください。"
        : String((rateLimitError as Error).message),
    }, unavailable ? 503 : 429);
  }

  try {
    const request = await readJsonBodyLimited<{
      provider?: unknown;
      endpoint?: unknown;
      body?: Record<string, any>;
    }>(req, MAX_REQUEST_BYTES);
    const { provider, endpoint, body } = request;
    const normalizedProvider = String(provider || "");
    const requestedModel = typeof body?.model === "string"
      ? body.model.trim()
      : "";
    const requestModel = requestedModel || DEFAULT_MODELS[normalizedProvider] ||
      null;
    apiLogger = new APILogger(
      normalizeApiName(normalizedProvider),
      "recipe-ai-proxy",
      requestModel,
    );
    apiLogger.setUser(userId, userEmail);
    ...
    if (!target) {
      ...
      return jsonResponse({
        success: false,
        error: "許可されていないプロバイダーまたはエンドポイントです。",
      }, 400);
    }
    ...
    if (
      ...
      (Array.isArray(body.tools) && body.tools.length > MAX_TOOLS)
    ) {
      if (apiLogger) {
        await apiLogger.logError("リクエスト項目数が上限を超えています。", {
          ...requestMetadata,
          proxy_stage: "validate_collection_limits",
        });
      }
      return jsonResponse({
        success: false,
        error: "メッセージまたはツールの件数が上限を超えています。",
      }, 413);
    }
    ...
    if (body.stream === true) {
      return jsonResponse({
        success: false,
        error: "ストリーミング形式には対応していません。",
      }, 400);
    }

    const allowedModels = ALLOWED_MODELS[normalizedProvider];
    if (!requestModel || !allowedModels?.has(requestModel)) {
      if (apiLogger) {
        await apiLogger.logError("許可されていないモデルです。", {
          ...requestMetadata,
          proxy_stage: "validate_model",
        });
      }
      return jsonResponse({
        success: false,
        error: "許可されていないAIモデルです。",
      }, 400);
    }

    const apiKey = target.keyEnvs.map((env) => Deno.env.get(env)).find((
      value,
    ) => value);
    if (!apiKey) {
      if (apiLogger) {
        await apiLogger.logError(
          `${target.providerName} のAPIキーがサーバーに設定されていません。`,
          {
            ...requestMetadata,
      ...
      return jsonResponse({
        success: false,
        error:
          `${target.providerName} のAPIキーがサーバーに設定されていません。管理者に連絡してください。`,
      }, 500);
    }
    ...
    if ("n" in upstreamBody) upstreamBody.n = 1;
    if (normalizedProvider === "openai") upstreamBody.store = false;
    for (
      const field of [
        "max_tokens",
      ...
      if (field in upstreamBody) {
        upstreamBody[field] = Math.min(
          Math.max(toPositiveInt(upstreamBody[field]), 1),
          MAX_OUTPUT_TOKENS,
        );
      }
    }
    if (normalizedProvider === "openai") {
      const hasJsonFormat =
        ...
        upstreamBody.text?.format?.type === "json_object";
      if (hasJsonFormat && Array.isArray(upstreamBody.tools)) {
        upstreamBody.tools = upstreamBody.tools.filter((t: any) =>
          t.type !== "web_search"
        );
    ...
    const openAiWebSearchCalls = normalizedProvider === "openai"
      ...
      : 0;
    const perplexitySearchContextSize = String(
    ...
    if (apiLogger) {
      const metadata = {
        ...
        billing_breakdown: billing
          ? {
            ...
            web_search_calls: "webSearchCalls" in billing
              ? billing.webSearchCalls
              : 0,
            web_search_cost_jpy: "webSearchCostJpy" in billing
              ? billing.webSearchCostJpy
              : 0,
            request_fee_jpy: "requestFeeJpy" in billing
              ? billing.requestFeeJpy
              : 0,
            search_context_size: "searchContextSize" in billing
              ? billing.searchContextSize
              : null,
      ...
      if (upstream.ok) {
        await apiLogger.logSuccess({
          ...
          estimatedCostJpy: billing?.knownPricing
            ? billing.totalCostJpy
            : undefined,
          metadata,
        });
