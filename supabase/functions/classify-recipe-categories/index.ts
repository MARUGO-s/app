...
const normalizeRecipeCategory = (
  ...
  if (!trimmed) {
    if (tags.some((tag) => /url取り込み|pdf取り込み|取り込み/i.test(tag))) {
      return "取り込み";
    }
    return "料理";
  }
  ...
  if (
    /スープ|soup|ポタージュ|potage|ビスク|bisque|コンソメ|consomme|ブイヨン|bouillon|汁物/
      .test(lower)
  ) {
    return "スープ";
  }
  ...
  if (
    /ソース・ドレッシング|ソース＆ドレッシング|ソース&ドレッシング/.test(
      trimmed,
    )
  ) {
    return "ソース・ドレッシング";
  ...
  if (/飾り|付け合わせ|ガーニッシュ|garnish/.test(lower)) {
    return "付け合わせ・飾り";
  }
  if (/デザート|お菓子|dessert|スイーツ|製菓/.test(lower)) {
    return "デザート・お菓子";
  }
...
serve(async (req) => {
  ...
  if (auth.mode === "user") {
    try {
      await enforceApiRateLimit(auth.userId, "classify-recipe-categories", {
        maxRequests: 30,
        windowMinutes: 60,
      });
    } catch (error) {
