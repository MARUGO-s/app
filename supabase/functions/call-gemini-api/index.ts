...
function buildErrorMessage(
  ...
  return `Gemini API error: ${status} ${statusText}`;
}
...
function extractFeature(body: RequestPayload | null): string | null {
  return readString(body?.logFeature) ||
    readString(body?.logContext?.feature) || null;
}

function normalizeOperationQaMetadata(
  body: RequestPayload,
  ...
  return {
    ...
    page_context: pageContext && typeof pageContext === "object"
      ? pageContext
      : null,
    ...extra,
  };
...
serve(async (req) => {
  ...
  try {
    ...
    const authenticatedUserId = readString(authPayload.sub);
    ...
    try {
      await enforceApiRateLimit(
        authenticatedUserId,
        "call-gemini-api",
        { maxRequests: 60, windowMinutes: 10 },
      );
    } catch (error) {
    ...
    if (!apiKey) {
      console.error(
        "❌ APIキーが見つかりません (GEMINI_API_KEY or GOOGLE_API_KEY)",
      );
    ...
    console.log(
      `[DEPLOY-CHECK-0640] APIキー使用中: ${
        geminiApiKey ? "GEMINI_API_KEY" : "GOOGLE_API_KEY"
      }`,
    );

    const endpointCandidates = buildGeminiGenerateContentEndpointCandidates(
      "v1beta",
    );
    const requestedModel = String(body.model || "").trim();
    if (requestedModel && requestedModel !== endpointCandidates[0]?.model) {
      console.warn(
        `最安モデル優先のため指定モデルを上書きします: ${requestedModel} -> ${
          endpointCandidates[0]?.model
        }`,
      );
    }

    const firstModelId = endpointCandidates[0]?.model ||
    ...
    requestSizeBytes =
      new TextEncoder().encode(JSON.stringify(geminiRequest)).length;
    ...
    for (let i = 0; i < endpointCandidates.length; i += 1) {
      ...
      if (!response.ok) {
        ...
        if (
          i < endpointCandidates.length - 1 &&
          isModelUnavailableError(response.status, errorText)
        ) {
          fallbackUsed = true;
          console.warn(
            `⚠️ Primary model unavailable, retry with fallback model: ${
              endpointCandidates[i + 1]?.model || "unknown"
            }`,
          );
          continue;
        }
        ...
        if (apiLogger) {
          const errorMetadata = {
            ...
            mode: body.mode || null,
            feature: readString(body.logFeature) ||
              readString(body.logContext?.feature) || null,
            source: readString(body.logContext?.source) || null,
            current_view: readString(body.logContext?.currentView) || null,
    ...
    try {
      ...
      if (apiLogger) {
        await apiLogger.logError(
          `GeminiレスポンスJSON解析に失敗: ${String(parseErr)}`,
          {
    ...
    console.log("✅ Gemini API レスポンス取得成功:", {
    ...
    try {
      ...
      recipeData = {
        title: "解析エラー",
        description:
          `Gemini APIからの応答を解析できませんでした: ${parseErrorMessage}`,
        servings: "1",
        ingredients: [],
        steps: [],
        notes: content,
    ...
    if (apiLogger) {
      await apiLogger.logSuccess({
        ...
        metadata: {
          mode: body.mode || null,
          feature: readString(body.logFeature) ||
            readString(body.logContext?.feature) || "general",
          source: readString(body.logContext?.source) || null,
          current_view: readString(body.logContext?.currentView) || null,
          assistant_mode: readString(body.logContext?.assistantMode) || null,
          attempted_models: attemptedModels,
    ...
    if (apiLogger && !logWritten) {
      await apiLogger.logError(
        error instanceof Error ? error.message : String(error),
        {
          ...
          feature: readString(requestBody?.logFeature) ||
            readString(requestBody?.logContext?.feature) || null,
          source: readString(requestBody?.logContext?.source) || null,
          current_view: readString(requestBody?.logContext?.currentView) ||
            null,
          assistant_mode: readString(requestBody?.logContext?.assistantMode) ||
            null,
        },
      );
    }
    ...
    const clientMessage = error instanceof HttpRequestError
      ? error.message
      : "AI処理中にエラーが発生しました。";
    return new Response(JSON.stringify({ ok: false, error: clientMessage }), {
      status,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
