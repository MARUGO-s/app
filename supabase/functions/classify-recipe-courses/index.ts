...
const normalizeRecipeCourse = (
  ...
  if (
    trimmed && (RECIPE_COURSE_OPTIONS as readonly string[]).includes(trimmed)
  ) {
  ...
  if (trimmed && CATEGORY_TO_COURSE_HINT[trimmed]) {
    return CATEGORY_TO_COURSE_HINT[trimmed];
  }
  if (category && CATEGORY_TO_COURSE_HINT[category]) {
    return CATEGORY_TO_COURSE_HINT[category];
  }
...
async function callGeminiClassify(
  items: ReturnType<typeof buildCompactRecipe>[],
) {
...
serve(async (req) => {
  ...
  const auth = await resolveRecipeClassifyClient(
    req,
    supabaseUrl,
    supabaseAnonKey,
    serviceRoleKey,
  ...
  if (auth.mode === "user") {
    try {
      await enforceApiRateLimit(auth.userId, "classify-recipe-courses", {
        maxRequests: 30,
        windowMinutes: 60,
      });
    } catch (error) {
  ...
  try {
    body = await readJsonBodyLimited<RequestBody>(req, MAX_REQUEST_BYTES);
  } catch (error) {
  ...
  let query = auth.client
    .from("recipes")
    .select(
      "id,title,description,course,category,servings,tags,ingredients,steps",
    )
