...
serve(async (req) => {
  ...
  try {
    await enforceApiRateLimit(userId, "call-document-intelligence", {
      maxRequests: 30,
      windowMinutes: 60,
    });
  } catch (error) {
  ...
  try {
    ...
    const requestBody = await readJsonBodyLimited<{
      image?: unknown;
      processorType?: unknown;
      aiProvider?: unknown;
    }>(req, MAX_REQUEST_BYTES);

    const { image, aiProvider = "groq" } = requestBody;

    if (typeof image !== "string" || !image) {
      throw new HttpRequestError("画像データが提供されていません。", 400);
    }
    if (aiProvider !== "groq" && aiProvider !== "chatgpt") {
      throw new HttpRequestError("AIプロバイダー指定が不正です。", 400);
    }
    ...
    if (
      !normalizedImage ||
      normalizedImage.length > MAX_IMAGE_BASE64_CHARS ||
      !/^[A-Za-z0-9+/]*={0,2}$/.test(normalizedImage) ||
      normalizedImage.length % 4 !== 0
    ) {
    ...
    if (!apiKey || !endpoint) {
      throw new Error(
        "Azure Document Intelligence の APIキー/エンドポイントが設定されていません（AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT / AZURE_DOCUMENT_INTELLIGENCE_KEY もしくは AZURE_DI_ENDPOINT / AZURE_DI_KEY）",
      );
    }

    console.log("🔑 Using Azure Document Intelligence API");

    console.log("🔄 Calling Document Intelligence API");
    ...
    if (!response.ok) {
      ...
      console.error(
        "❌ Azure Document Intelligence API エラー:",
        response.status,
      );
      throw new Error(
        `Azure Document Intelligence API エラー: ${response.status}`,
      );
    }
    ...
    try {
      ...
      if (response.status === 202) {
        ...
        if (responseText.trim()) {
          ...
          console.log("📄 Azure Document Intelligence API レスポンス取得成功");
        } else {
      ...
      throw new Error(`JSON解析エラー: ${getErrorMessage(jsonError)}`);
    }
    ...
    try {
      ...
      // AI APIでテキストを解析
      const aiRecipeData = await callAIForTextAnalysis(
        extractedText,
        aiProvider,
        aiApiKey,
      );
      ...
      if (aiRecipeData) {
        ...
        console.log(`✅ ${aiProvider} API解析完了`);
      } else {
      ...
      console.error(
        `❌ ${aiProvider} API処理エラー:`,
        aiError instanceof Error ? aiError.name : "unknown",
      );
      throw new Error(`AI解析に失敗しました: ${getErrorMessage(aiError)}`);
    }
...
// AI APIでテキストを解析する関数
async function callAIForTextAnalysis(
  text: string,
  aiProvider: string,
  apiKey: string,
): Promise<any> {
  ...
  if (aiProvider === "groq") {
    ...
    if (response.ok) {
      ...
      await response.body?.cancel();
      console.error("❌ Groq API エラー:", response.status);
    ...
    if (response.ok) {
      ...
      await response.body?.cancel();
      console.error("❌ ChatGPT API エラー:", response.status);
      throw new Error(`ChatGPT API エラー: ${response.status}`);
    }
  }
...
function parseAIJsonResponse(content: string, originalText: string): any {
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      console.warn("⚠️ JSON解析失敗候補:", { length: jsonText.length });
    }
  }
...
function fallbackRecipeFromText(text: string): any {
  ...
  const ingredients: Array<
    { item: string; quantity: string; unit: string; price?: string }
  > = [];
  const steps: string[] = [];

  const unitPattern =
    /(?:g|kg|ml|L|cc|個|本|枚|カップ|大さじ|小さじ|杯|片|切れ|缶|袋)/;

  lines.forEach((line) => {
    if (
      /^\d+[\.、\)]/.test(line) ||
      /工程|手順|STEP|混ぜ|焼|煮|炒め|茹で/.test(line)
    ) {
    ...
    const approximateMatch = line.match(
      /^(.*?)[：:\s]*?(適量|少々|お好み|ひとつまみ|お好みで)(?:[\s).）]*.*)?$/,
    );
    if (approximateMatch) {
      const item = approximateMatch[1]
        ...
        .replace(/[:：]/g, "")
        .trim() ||
        line.replace(/(適量|少々|お好み|ひとつまみ|お好みで).*/g, "").trim();
    ...
    const priceOnlyMatch = line.match(
      /^(.*?)[：:\s]*?(¥\s*\d+(?:[,.]\d+)?|\d+(?:[,.]\d+)?\s*円)(?:\s|$)/,
    );
    if (priceOnlyMatch) {
...
function removeUnitSuffixFromItem(
  item: string,
): { item: string; detectedUnit: string | null } {
...
function normalizeIngredientEntry(
  raw: any,
): { item: string; quantity: string; unit: string; price: string } {
  if (!raw) {
  ...
  if (typeof raw === "string") {
    return normalizeIngredientEntry({
      item: raw,
      quantity: "",
      unit: "",
      price: "",
  ...
  const matchedRule = UNIT_NORMALIZATION_TABLE.find(({ pattern }) =>
