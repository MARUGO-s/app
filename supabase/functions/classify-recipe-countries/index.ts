...
serve(async (req) => {
  ...
  if (auth.mode === "user") {
    try {
      await enforceApiRateLimit(auth.userId, "classify-recipe-countries", {
        maxRequests: 30,
        windowMinutes: 60,
      });
    } catch (error) {
  ...
  let query = auth.client
    .from("recipes")
    .select(
      "id,title,description,course,category,servings,country,ingredients,steps",
    )
  ...
  const apiLogger = new APILogger(
    "gemini",
    "classify-recipe-countries",
    "gemini-3.1-flash-lite",
  );
  const allResults: Array<
    ClassificationResult & { saved?: boolean; error?: string }
  > = [];
  let updated = 0;
  let failed = 0;
