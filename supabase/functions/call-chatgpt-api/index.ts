...
serve(async (req) => {
  ...
  try {
    await enforceApiRateLimit(userId, "call-chatgpt-api", {
      maxRequests: 60,
      windowMinutes: 10,
    });
  } catch (error) {
  ...
  try {
    ...
    const modelId = Deno.env.get("OPENAI_CHAT_MODEL") || DEFAULT_MODEL;
    ...
    console.log("✅ ChatGPT API レスポンス取得成功:", {
...
async function analyzeRecipeWithChatGPT(
  text: string,
  url?: string,
): Promise<any> {
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "あなたはレシピ解析の専門家です。与えられたテキストからレシピ情報を正確に抽出し、指定されたJSON形式で返してください。",
        },
        {
          role: "user",
          content: prompt,
