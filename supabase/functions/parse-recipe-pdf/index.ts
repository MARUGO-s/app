...
const MAX_REQUEST_BYTES = MAX_PDF_BASE64_CHARS + 64 * 1024;
...
const parseRecipesPayload = (
  raw: string,
): { recipes: unknown[]; partial: boolean } => {
...
async function callGemini(prompt: string, pdfBase64: string) {
  ...
  try {
      ...
      const modelUnavailable = current.status === 404 ||
        (current.status === 400 &&
          (lowered.includes("not found") ||
            (lowered.includes("model") && lowered.includes("supported"))));
      const schemaUnsupported = current.status === 400 &&
        (lowered.includes("responsemimetype") ||
          lowered.includes("responseschema") ||
          lowered.includes("unknown name"));

      if (
        i < candidates.length - 1 && (modelUnavailable || schemaUnsupported)
      ) {
        console.warn(
          `⚠️ Gemini request failed for ${candidate.model}, trying fallback`,
        );
        continue;
      }
    ...
    if ((e as Error)?.name === "AbortError") {
      throw new Error(
        "Gemini API がタイムアウトしました。ページ数の少ないPDFで再試行してください。",
      );
    }
    throw e;
  } finally {
  ...
  if (!res) {
    const t = lastError
      ? `${lastError.status} ${lastError.statusText} ${lastError.body}`
      : "Unknown error";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      await enforceApiRateLimit(userId, "parse-recipe-pdf", {
        maxRequests: 10,
        windowMinutes: 60,
      });
    } catch (error) {
      ...
      return new Response(
        JSON.stringify({ ok: false, error: "利用制限を確認できません。" }),
        {
          status: 503,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const fileBase64 = stripDataUrlPrefix(
      String(body?.fileBase64 || body?.pdfBase64 || ""),
    );
    ...
    if (
      !fileBase64 ||
      fileBase64.length > MAX_PDF_BASE64_CHARS ||
      !/^[A-Za-z0-9+/]*={0,2}$/.test(fileBase64) ||
      fileBase64.length % 4 !== 0
    ) {
      throw new HttpRequestError(
        "PDFデータが空か、形式またはサイズが不正です。",
        400,
      );
    }
    ...
    const { recipes: rawRecipes, partial: salvaged } = parseRecipesPayload(
      geminiText,
    );
    ...
    if (recipes.length === 0) {
      throw new Error(
        "PDFからレシピを抽出できませんでした。別のPDFか、画像が鮮明なファイルで再試行してください。",
      );
    }
    ...
    const safeMessage = error instanceof HttpRequestError
      ? error.message
      : error instanceof Error && /^(PDFから|JSON解析)/.test(error.message)
      ? error.message
      : "PDF解析中にエラーが発生しました。";
    const status = error instanceof HttpRequestError ? error.status : 422;
    return new Response(
      JSON.stringify({
        ok: false,
        error: safeMessage,
