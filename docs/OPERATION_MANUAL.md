...
- レシピ詳細の**分量表示**を見直し、再計算時の数量表示を小数1桁（四捨五入）で統一。
- 分数入力（例: `1/8`）の扱いを改善し、分量集計で `parseFloat("1/8") => 1` になる誤集計を修正。
- カテゴリ集計モーダルの「今回使う量」を再読込後も保持するよう改善。
- 材料セクションに**総量表示**（重量g・液量ml）を追加。
- グループ見出しの「セット原価」に、`g + ml` を `1ml=1g` 概算した**合算g**を併記。
- 原価率シミュレーターを強化し、**使用量(g)ベース原価率**を主表示に変更。
- レシピ全体の**歩留まり入力欄**を材料セクションに新設（シミュレーター内入力から移設）。
- 歩留まり値をレシピメタデータに保存し、再読込時に復元されるよう改善。
...
合成原価は、複数レシピ・単体材料を組み合わせた商品原価を確認する画面です。

- **原価インパクト分析**
  - 合成原価合計の下に、使用原価が高い順で行別の割合を表示します。
  - 原価改善時は、上位に出るレシピ/材料から使用量・材料差し替え・カテゴリ別原価を確認してください。
- **目標原価率から使用量を逆算**
  - 販売価格と目標原価率を入力し、対象行を選ぶと、その行を何g/何ml/何個まで使えるか逆算できます。
  - `この使用量を反映` で、算出された上限使用量を使用量欄へ反映できます。
- **保存済み一覧の原価変動バッジ**
  - 保存時の合成原価と、現在の材料単価・カテゴリ別原価で再計算した原価との差を表示します。
  - 価格CSV更新後やカテゴリ別原価の再設定後は、保存済み一覧で `原価変動` を確認してください。
