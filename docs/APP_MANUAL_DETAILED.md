...
- **カード**: サムネイル付きカード。標準の見やすい表示。
- **リスト**: レシピ名・店舗・コース・カテゴリー・国・登録日をテーブル表示（画像なし・軽量）。件数が多い一覧向け。
- 切り替えはフィルター右の **カード / リスト** ボタン。選択はブラウザに記憶されます。

### 4.4 表示件数（通常 / 全表示）

- **通常**: コース別セクションごとに最大8〜9件（リスト時は24〜30件）まで表示。「▼ もっと見る」で展開。
- **全表示**: セクション内のレシピをすべて表示（折りたたみ・もっと見るなし）。カード/リストの見た目はそのまま。

### 4.5 一覧件数と表示の関係

- タイトルの `(21)` などは、検索・フィルタ後に取得できた**総件数**です。
- **自分のレシピ**と**マスターレシピ共有**（ユーザー管理でONの場合）は、メインのコース別一覧に出ます。
- **他ユーザーの公開レシピ**は 🌐「他ユーザー公開」で別枠表示します。

### 4.6 公開レシピ（自分公開中 / 他ユーザー公開）
...
加えて、価格データタブ内にレシピのバックアップ（JSON）の入出力もあります。
...
### 9.2 バックアップ管理（管理者・アカウント別）

**データ管理 → バックアップ管理**（管理者のみ）

目的:

- 全ユーザーのレシピをサーバー側に保存し、障害時や誤操作時に復元できるようにします。

自動バックアップ:

- **毎週1回**、**月曜 0:00 JST**（日曜 UTC 15:00）に全ユーザー分を実行
- ユーザーごと **最大3世代**（第1〜第3世代。古い世代から上書き）
- 一覧のラベルは「自動バックアップ（定期）」

手動バックアップ:

- **「今すぐバックアップ」**（全体またはユーザー単位）で即時実行
- ラベルは「手動バックアップ」
...
- **確認** … レシピ一覧のプレビュー
- **DL** … JSON ダウンロード
- **復元** … 当該ユーザーのレシピをバックアップ内容で復元（要確認）

注意:

- JSON 取り込み（9.1）とは別機能です。JSON は手動エクスポート、本機能は全アカウントの世代管理です。
- 詳細・cron 設定は [アカウント別バックアップ](./account-backup.md) を参照
