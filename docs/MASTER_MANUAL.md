...
### 5-4. バックアップ管理（管理者・アカウント別）

管理者は **データ管理 → バックアップ管理** で、全ユーザーのレシピをサーバーに世代保存できます。

- **自動:** **毎週1回**（月曜 0:00 JST）。ユーザーごと最大3世代
- **手動:** 「今すぐバックアップ」で即時実行
- **復元 / DL:** 画面から世代ごとに操作

詳細は [アカウント別バックアップ](./account-backup.md) を参照してください。
...
### 14-5. 合成レシピ編集で「詳細」から戻る運用（2026-05-01 追加）

- 合成レシピ編集画面の各レシピ行に `詳細` ボタンを追加しました。
- `詳細` で開いた先のレシピを編集しても、`← 合成レシピ編集に戻る` でワンクリック復帰できます。
- 共有レシピを編集して複製保存になった場合でも、戻った合成画面では新しい複製レシピIDを参照するよう自動更新されます。

### 14-6. `recent_views` の 409 エラーについて（2026-05-01 対策済み）

- 以前は最近閲覧履歴の保存で `recent_views` に 409/400 が出ることがありました。
- 現在は保存ロジックを「存在確認 -> update/insert（競合時のみ再update）」に変更し、エラー発生を大幅に抑制しています。
- まれに同時操作で一時的な競合が起きることはありますが、通常運用では問題なく履歴が保存されます。

### 14-7. 合成原価の分析・逆算・変動確認（2026-05-02 追加）

- 合成原価合計の下に `原価インパクト分析` を追加しました。
  - 使用原価が高い順に、レシピ/材料名、使用量、使用原価、合成原価に占める割合を表示します。
  - どの行から量目調整・材料差し替え・カテゴリ原価確認を行うべきか判断しやすくなります。
- `合成原価率シミュレーター` に `目標原価率から使用量を逆算` を追加しました。
  - 販売価格と目標原価率を入力し、対象行以外の原価を固定したうえで、選択行の上限使用量を逆算します。
  - `この使用量を反映` で算出値を対象行の使用量欄へ反映できます。
- 保存済み合成レシピ一覧に `原価変動` バッジを追加しました。
  - 保存時原価と現在の材料単価・カテゴリ別原価で再計算した原価との差額/変動率を表示します。
  - `差分なし`、上昇、低下、`取得不可`、`一部取得不可` をカード上で判別できます。
