...
過去の公開Git履歴に含まれていた認証seedや診断用資格情報は、現在ツリーから削除しても過去履歴には残る。関連ユーザーのパスワード変更、全セッション失効、該当キーのローテーションを実施し、必要なら履歴書換えを別の事故対応として行う。

旧 `app_users` のpassword／秘密の質問データもブラウザからは参照不能だが、本番DBからの物理削除はまだ行わない。影響確認と必要な退避を行ったうえで、上記資格情報事故対応と同時に削除する。
