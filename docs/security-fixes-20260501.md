...
### 問題
特定ユーザーの実メールアドレスがソースコードに直接書かれており、そのユーザーに特権が与えられていた。
...
```js
// 修正前
const isSuperAdmin = email === '<legacy-email>';
...
```js
// 修正前
const isSuperAdmin = (u) => u?.email === '<legacy-email>';
...
const supabaseAnonKey = getEnv('VITE_SUPABASE_ANON_KEY') || '<PUBLISHABLE_KEY>';
```
