...
### 2026-05-02 追加実装メモ（合成原価の分析・逆算・変動確認）

#### 1. 原価インパクト分析

- 対象: `RecipeCompositeCostCalculator`
- `currentLine` と `otherLines` から `lineSummaries` を構築し、行ごとの使用原価と合成原価に占める割合を算出。
- `costImpactRows` は使用原価が有効な行だけを抽出し、金額の降順で表示。
- 表示内容:
  - レシピ/材料名
  - 種別（ベース / レシピ / 材料）
  - 使用量
...
- 計算:
  1. `許容原価 = 販売価格 × 目標原価率`
  2. `対象外の原価 = 対象行以外の使用原価合計`
  3. `対象行に使える原価 = 許容原価 - 対象外の原価`
  4. レシピ行は `対象行に使える原価 ÷ 1g/1mlあたり原価` で使用量を算出。
  5. 材料行は `costFromQuantityAndUnit` の単位換算と税率を逆向きにして使用量を算出。
- `この使用量を反映` で、算出値をベース行または対象行の `usageAmount` に反映する。
- 対象行以外の原価だけで目標を超える場合は、使用量反映を無効にする。

#### 3. 保存済み一覧の原価変動バッジ

- 対象: `RecipeCompositeSavedListPage`
- 一覧取得後、各保存済み合成レシピに対して `getSetDetail`、参照レシピ、カテゴリ別原価 override を読み込み、`computeCompositeSnapshotTotals` で現在原価を再計算。
- 保存時原価 `total_cost_tax_included` と現在再計算原価を比較し、差額・変動率を `driftById` に保持。
- 表示状態:
  - `差分なし`
  - `+¥... (+...%)` 原価上昇
  - `-¥... (-...%)` 原価低下
  - `取得不可`
  - `一部取得不可`
- DBスキーマ変更は不要。保存済みデータと既存の再計算ユーティリティのみを使用。
