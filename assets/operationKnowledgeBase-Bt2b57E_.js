const n=`import operationGuideKnowledge from '../data/operationGuideKnowledge.json';
...
const CORE_OPERATION_KB = [
    ...
    },
    {
        ...
        steps: [
            ...
            '解析エンジンを選択します（通常は「標準（OpenAI Vision優先）」、手書きは「手書き（Gemini）」を試す）。',
    ...
    },
    {
        ...
        steps: [
            ...
            '通常は「標準（OpenAI Vision優先）」、手書きや崩し字が多い場合は「手書き（Gemini）」を使います。',
    ...
    },
    {
        ...
        title: 'レシピを検索・絞り込みする',
        keywords: ['検索', '絞り込み', 'フィルタ', 'タグ', 'コース', 'カテゴリー', '店舗', '国', '通常', '全表示', 'カード', 'リスト', '自分公開中', '他ユーザー公開', '公開非表示'],
        views: ['list'],
        steps: [
            '一覧上部の検索欄にキーワードを入力します。',
            '店舗・コース・カテゴリー・国のプルダウンで条件を選びます。',
            '「カード/リスト」で見た目を切り替えます（リストは画像なしのテーブル表示）。',
            '「通常/全表示」でセクション内の表示件数を切り替えます（全表示でもカード表示は維持されます）。',
            '公開レシピを見たい場合は「🟢 自分公開中」または「🌐 他ユーザー公開」を押します。',
            '公開一覧を閉じるときは「✕ 公開非表示」を押します。'
        ],
        notes: '条件が多いと0件表示になりやすいので、結果が出ないときは条件を1つずつ外して確認します。カード/リストの選択はブラウザに保存されます。'
    },
    {
        id: 'recipe_list_layout_mode',
        title: 'レシピ一覧のカード表示とリスト表示を切り替える',
        keywords: ['カード', 'リスト', 'リスト表示', 'テーブル', '一覧表示', 'レイアウト', 'サムネイル', '画像なし', '表示 切り替え', 'recipe-list-layout'],
        views: ['list'],
        steps: [
            'レシピ一覧を開き、フィルター右側の「カード」「リスト」ボタンを確認します。',
            'カード: サムネイル付きの標準表示です。',
            'リスト: レシピ名・店舗・コース・カテゴリー・国・登録日をテーブル表示します（画像なしで軽量）。',
            '「通常/全表示」と併用できます。最後に選んだレイアウトはブラウザに記憶されます。'
        ],
        notes: '全表示ボタンは件数の展開用です。リスト表示への切り替えは「リスト」ボタンで行います。'
    },
    {
        id: 'recipe_list_count_mismatch',
        title: '件数は多いのにレシピカードが少ない・表示が足りない',
        keywords: ['件数', '表示されない', '少ない', 'カードが出ない', '一覧 件数', 'マスターレシピ', 'マスター共有', 'show_master', '一般ユーザー', '21件', 'もっと見る'],
        views: ['list'],
        steps: [
            'タイトルの (数字) は検索・フィルタ後に取得できた総件数です（画面に一度に出る件数とは別です）。',
            '自分のレシピはコース別のメイン一覧に表示されます。',
            'マスターレシピ共有がONの場合、管理者のマスターレシピもメイン一覧に含まれます（ユーザー管理で設定）。',
            '他ユーザーの公開レシピは「🌐 他ユーザー公開」ボタンで別枠に表示します。',
            'セクション内が少ない場合は「全表示」を押すか、「▼ もっと見る」で展開します。'
        ],
        notes: '全表示はリスト/カードの見た目ではなく件数展開の切り替えです。マスター共有OFFの一般ユーザーは自分＋公開レシピが対象です。'
    },
    {
    ...
    // ── レシピ一覧の細かい挙動 ─────────────────────────────────
    {
        ...
        views: ['list'],
        steps: [
            'レシピはコース（前菜・メイン・パン等）ごとのセクションに分類されます。',
            '「通常」モード: カード表示はPC9件/スマホ8件、リスト表示は24〜30件まで。超えると「▼ もっと見る」が出ます。',
            '「全表示」モード: セクション内をすべて一度に表示（カード/リストの見た目はそのまま）。',
            '「カード/リスト」ボタンで見た目を切り替えられます（全表示とは別機能）。',
            '自分のレシピとマスター共有レシピはメイン一覧、他ユーザーの公開は「🌐 他ユーザー公開」枠です。'
        ],
        notes: '全表示は件数展開のみで、リスト表示への切り替えではありません。タグに「パン」等が含まれるレシピはパン系コースに分類されます。'
    },
    {
...
const SYNONYM_GROUPS = [
    ...
    ['セクション', 'カテゴリ一覧', 'もっと見る', '閉じる', 'パン一覧', 'デザート一覧', 'ソース一覧', 'ドレッシング一覧', 'コース 一覧', '料理 分類', 'カード リスト', 'リスト表示', '全表示 カード', '表示件数'],
...
const ENTRY_INTENT_BOOSTS = {
    ...
    recipe_search_filter: ['検索', '絞り込み', 'フィルタ', 'カード', 'リスト', '通常', '全表示'],
    recipe_list_layout_mode: ['カード', 'リスト', 'リスト表示', 'テーブル', '一覧表示', 'レイアウト', 'サムネイル'],
    recipe_list_count_mismatch: ['件数', '表示されない', '少ない', 'カードが出ない', 'マスターレシピ', 'マスター共有', 'もっと見る', '全表示 違い'],
...
const buildClarificationLines = ({ assessment, currentViewLabel, currentView }) => {
    ...
    const buttonHintsByView = {
        ...
        list: '検索欄 / カード / リスト / 通常 / 全表示 / + レシピ追加 / 🟢 自分公開中 / 🌐 他ユーザー公開 / ☰ メニュー',
    };
