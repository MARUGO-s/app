...
(function(){
  ...
  function renderDetail(){
    ...
    (async()=>{ const btn=$('#favToggle',detailEl); if(!btn) return; if(!r?.id){ btn.disabled=true; btn.textContent='♡'; btn.title='保存すると有効になります'; return; }
  ...
  async function isFav(recipe_id){ if(!recipe_id) return false; const { data }=await sb.from('favorites').select('id').eq('client_id', identity.client_id).eq('recipe_id', recipe_id).limit(1); return !!(data&&data[0]); }
  async function setFav(recipe_id, on){ if(!recipe_id) return false;
    if(on){
