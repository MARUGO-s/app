...
// assets/js/app.supabase.js — secure-only（要ログイン）
(function(){
  'use strict';
  ...
  // ===== identity & auth =====
  const id = { uid:null };
  sb.auth.getUser().then(({data})=> id.uid = data?.user?.id || null);
  sb.auth.onAuthStateChange((_e, sess)=>{ id.uid = sess?.user?.id || null; refreshAll(); });
  ...
  // ===== queries =====
  function needLogin(){ if(!id.uid){ alert('ログインが必要です'); return true; } return false; }
  async function qRecipesLite(){ if(needLogin()) return []; const { data } = await sb.from('recipes').select('id,title,tags,updated_at').eq('user_id', id.uid).order('updated_at',{ascending:false}).limit(200); return data||[]; }
  async function qRecipeFull(idv){ if(needLogin()) throw new Error('login'); const { data: r } = await sb.from('recipes').select('*').eq('id',idv).single(); const { data: ings } = await sb.from('recipe_ingredients').select('*').eq('recipe_id',idv).order('position'); const { data: steps } = await sb.from('recipe_steps').select('*').eq('recipe_id',idv).order('position'); return { recipe:r, ings:ings||[], steps:steps||[] }; }
  async function qFavList(){ if(needLogin()) return []; const { data } = await sb.from('favorites').select('recipe_id, recipes!inner(id,title,tags,updated_at)').eq('user_id', id.uid).order('created_at',{ascending:false}); return (data||[]).map(x=>x.recipes); }
  async function qCounts(){
    if(needLogin()) return {total:0,fav:0,tags:0,recent:[]};
    const [{ data: r1 }, { data: r2 }] = await Promise.all([
      sb.from('recipes').select('id,tags,updated_at').eq('user_id', id.uid),
      sb.from('favorites').select('id').eq('user_id', id.uid),
    ]);
  ...
  async function loadOverview(){
    ...
    if(!s.recent.length){ recentEl.classList.add('empty'); recentEl.textContent='まだレシピがありません'; }
      ...
      recentEl.classList.remove('empty');
      s.recent.forEach(r=>{
    ...
    // home favorites
    const favs = await qFavList(); const homeFav = $('#homeFavList'); homeFav.innerHTML='';
    if(!favs.length){ homeFav.classList.add('empty'); homeFav.textContent='なし'; }
    else{
      homeFav.classList.remove('empty');
      favs.slice(0,5).forEach(r=>{
        const card=document.createElement('div'); card.className='card';
        card.innerHTML=`<div class="t">${esc(r.title)}</div>`;
        card.addEventListener('click',()=>{ show('recipes'); openRecipe(r.id); });
        homeFav.appendChild(card);
      });
    }
  }
  ...
  function renderList(items){
    ...
    arr.forEach(r=>{ const card=document.createElement('div'); card.className='card';
      card.innerHTML=`<div class="t">${esc(r.title)}</div>
        <div class="meta">${(r.tags||[]).map(t=>`<span class="tag">${esc(t)}</span>`).join(' ')}</div>`;
  ...
  async function openRecipe(idv){
    try{ state.current = await qRecipeFull(idv); renderDetail(); }catch(e){}
  }
  function renderDetail(){
    ...
    (async()=>{ const btn=$('#favToggle',detailEl); if(!btn) return;
      if(!id.uid){ btn.addEventListener('click',()=>alert('お気に入りはログインが必要です')); return; }
      let on = await isFav(r.id); btn.textContent = on?'♥':'♡';
      btn.addEventListener('click', async ()=>{ on=!on; const ok=await setFav(r.id,on); if(!ok){ on=!on; return; } btn.textContent = on?'♥':'♡'; loadOverview(); if((location.hash||'#').slice(1)==='fav') loadFav(); });
    })();
  }
  ...
  async function isFav(recipe_id){ if(needLogin()) return false; const { data, error }=await sb.from('favorites').select('id').eq('user_id',id.uid).eq('recipe_id',recipe_id).limit(1); if(error){ return false; } return !!(data&&data[0]); }
  async function setFav(recipe_id, on){ if(needLogin()) return false; if(on){ const { error }=await sb.from('favorites').insert({ user_id:id.uid, recipe_id }); if(error && error.code!=='23505'){ alert('追加に失敗: '+error.message); return false; } return true;} else { const { error }=await sb.from('favorites').delete().eq('user_id',id.uid).eq('recipe_id',recipe_id); if(error){ alert('削除に失敗: '+error.message); return false; } return true; } }

  // ===== Settings =====
  $('#search')?.addEventListener('input', e=>{ state.search=e.target.value||''; renderList(state.list); });
  $('#btnNew')?.addEventListener('click', ()=>{ if(needLogin()) return; state.current={ recipe:{title:''}, ings:[], steps:[] }; show('recipes'); renderDetail(); });

  // export/import/dummy
  $('#btnExport')?.addEventListener('click', async ()=>{
    if(needLogin()) return;
    const { data: recipes } = await sb.from('recipes').select('*').eq('user_id', id.uid).order('updated_at',{ascending:false});
  ...
  function scaleFont(d){ const r=document.documentElement; const cur=parseFloat(getComputedStyle(r).getPropertyValue('--fs')||'1')||1; r.style.setProperty('--fs', String(Math.max(0.8, Math.min(1.3, cur + d)))); }
  ...
  async function refreshAll(){ const v=(location.hash||'#home').slice(1); show(v); }
  // initial
