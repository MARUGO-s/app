...
const addStepRow = (data = {}) => {
  ...
  div.innerHTML = `
    <textarea placeholder="手順を入力してください" class="step-text">${escapeHtml(data.instruction || '')}</textarea>
    <button type="button" class="btn primary small js-remove-row">削除</button>
  `;
  container.appendChild(div);
...
const showAISaveOptions = () => {
  ...
  if (currentRecipeId) {
    ...
    const choice = confirm(
      'AI創作が完了しました！\n\n' +
      'OK: 現在のレシピを上書き保存してトップページに戻る\n' +
      'キャンセル: 新規レシピとして保存してトップページに戻る'
    );
    
    console.log('ユーザーの選択:', choice ? '上書き保存' : '新規保存');
